...
public class TransformCalculator {
	...
	public static void main(String[] args) {
		...
		Matrix4d matrix = new Matrix4d(); // initialize identity matrix
		...
		if (sargs.length != 3) { // check length
			...
			try { // ensure all arguments after first are doubles
				for (int i = 0; i < args.length; i++) {
					args[i] = Double.parseDouble(sargs[i]); // make casts
				}
			} catch (Exception e) {
	...
	public static Matrix4d stringTransform(String s, Matrix4d m) throws IllegalArgumentException {
		...
		double[] args = new double[sargs.length - 1]; // skip transform id
		try { // ensure all arguments after first are doubles
			for (int i = 0; i < args.length; i++) {
		...
		case "L":
			if (args.length == 6) {
				return m.lookAlong(args[0], args[1], args[2], args[3], args[4], args[5]); // look at
														// (0,0,0)
			} else {
				invalid = 6;
			}
